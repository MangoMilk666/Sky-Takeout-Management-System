...
export function DishPage() {
  ...
  const [dishStatus, setDishStatus] = useState<number | undefined>(undefined)
  ...
  const fetchList = async (opts?: {
    ...
    try {
      const res = await getDishPage({
        ...
        status: opts?.statusOverride ?? dishStatus,
      })
  ...
  const columns = useMemo(
    () => [
      ...
      { title: '菜品分类', dataIndex: 'categoryName' },
      {
        title: '售价',
        dataIndex: 'price',
      ...
      { title: '最后操作时间', dataIndex: 'updateTime' },
      {
        ...
        key: 'actions',
        width: 240,
        render: (_: any, row: DishRow) => {
          const nextStatus = String(row.status) === '0' ? '1' : '0'
          return (
            <Space>
              <Button type="link" onClick={() => navigate(`/dish/add?id=${encodeURIComponent(row.id)}`)}>
                修改
              </Button>
              <Button
                type="link"
              ...
              <Button
                ...
                onClick={() => {
                  Modal.confirm({
                    ...
                    cancelText: '取消',
                    onOk: async () => {
                      const res = await dishStatusByStatus({ id: row.id, status: nextStatus })
                      if (String(res.data?.code) === '1') {
    ...
    [navigate, categoryId, dishStatus, keyword, page, pageSize],
  )
  ...
  return (
    <Card>
        ...
        <Space>
          <Button
            ...
            onClick={() => {
              ...
              Modal.confirm({
                title: '确定删除',
                content: '确认删除所选菜品, 是否继续?',
                okText: '删除',
                cancelText: '取消',
                onOk: async () => {
                  const res = await deleteDish(selectedIds.join(','))
                  if (String(res.data?.code) === '1') {
                    message.success('删除成功！')
                    setSelectedIds([])
                    await fetchList()
      ...
      <Table<DishRow>
        ...
        locale={{
          emptyText: isSearch ? '未搜索到相关菜品' : '暂无数据',
        }}
      />
    </Card>
  )
